...
function partThreeB () {
  ...
  // 2. add the friend to the message
  messageForSplitBill.Paragraph.innerHTML += `, ${friend}`;
}
...
let newFavFlavor = prompt("Enter your favorite ice cream flavor:");
  // 2. add the flavor to the array with push()
myFavFlavors.push(newFavFlavor);
  // 3. add the flavor to the message
alert(`You added "${newFavFlavor}". Here are your favorite flavors so far: ${myFavFlavors.join(" ,")}`);
  // 4. log the array to the console for debugging
console.log("Current favorite flavors:", myFavFlavors);
}
